    ...
    const std::shared_ptr<MessageThread> &thread, const MessagePtr &parent)
{
    ...
    if (getSettings()->allowBridgeImpersonation &&
        builder->loginName == getSettings()->bridgeUser)
    {
        // bridged message:
        // [I] <${user}> ${text}
        QRegularExpression bridgeMeta(R"(^(\[.\]) <(.+?)> (.*)$)");
        auto m = bridgeMeta.match(content);
        if (m.hasMatch())
        {
            auto platform = m.captured(1);
            auto user = m.captured(2);
            content = m.captured(3);
            builder->loginName = user + platform;
            isBridged = true;
        }
    }
        ...
        !isBridged)
    {
        ...
        if (isAbnormal && getSettings()->abnormalNonceDetection)
        {
            ...
            builder.emplace<TextElement>(
                ...
                MessageColor::System);
            builder.emplace<TextElement>(tags["client-nonce"].toString(),
                                         MessageElementFlag::ChannelPointReward,
                                         MessageColor::Text);
